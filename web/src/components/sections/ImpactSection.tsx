    ...
    return (
        <section className="bg-neutral-900 text-white overflow-hidden relative">
            ...
            <div className="container mx-auto px-6 py-24 relative z-10 text-center">
                ...
                {/* 2. Key Metrics Grid */}
                {content.keyMetrics && (
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mt-16 md:mt-20">
                        {content.keyMetrics.map((metric, i) => (
                            <motion.div
                                ...
                                className="bg-white/5 border border-white/10 p-3 sm:p-4 md:p-6 lg:p-8 rounded-lg hover:border-[var(--yarrow)]/50 transition-colors group relative"
                            >
                                <div className="absolute inset-0 bg-gradient-to-br from-[var(--yarrow)]/0 to-[var(--yarrow)]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-lg" />
                                <h3 className="text-lg sm:text-xl md:text-2xl lg:text-4xl xl:text-5xl font-bold text-[var(--yarrow)] mb-1 md:mb-2 relative z-10">
                                    <CountUp value={metric.value} prefix={metric.prefix} suffix={metric.suffix} />
                                </h3>
                                <p className="text-neutral-300 font-medium tracking-wide text-[10px] sm:text-xs md:text-sm relative z-10 leading-tight">
                                    {t(metric.label)}
                                </p>
                            </motion.div>
                        ))}
